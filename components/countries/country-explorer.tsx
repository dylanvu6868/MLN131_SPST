...
import { displayRegion, regimeLabel, tr } from "@/lib/i18n";
    ...
    <div className="space-y-5">
      <div className="atlas-surface rounded-lg p-4">
        <div className="grid gap-3 lg:grid-cols-[1fr_180px_220px_190px]">
          ...
          <label className="block">
            <span className="sr-only">{tr("Dấu hiệu nhà nước")}</span>
            <select
              value={stateFlag}
              onChange={(event) => setStateFlag(event.target.value)}
              className="atlas-input w-full rounded-md px-3 py-2"
            >
              <option value="">{tr("Tất cả dấu hiệu")}</option>
              <option value="communist">{tr("Đảng cộng sản cầm quyền")}</option>
              <option value="monarchy">{tr("Quân chủ")}</option>
              <option value="federal">{tr("Liên bang")}</option>
              <option value="unitary">{tr("Đơn nhất")}</option>
            </select>
          </label>
        </div>
        ...
        <div className="mt-4 flex items-center gap-2 text-sm text-slate-400">
          ...
          {filtered.length} / {countries.length} {tr("hồ sơ")}
        </div>
      </div>
